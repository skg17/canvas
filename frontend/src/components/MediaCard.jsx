    ...
    <div className="media-card bg-[#17131D] rounded-[14px] overflow-hidden shadow-[0_4px_12px_rgba(0,0,0,0.3)] transition-all hover:-translate-y-0.5 hover:shadow-[0_8px_20px_rgba(0,0,0,0.4)]">
      <div className="card-poster relative w-full">
        ...
        <div className={`card-menu absolute top-2 right-2 sm:top-3 sm:right-3 z-10 ${menuOpen ? 'active' : ''}`}>
          <button
            className={`card-menu-toggle bg-black/70 backdrop-blur-sm text-text-primary border-none rounded-lg w-9 h-9 sm:w-8 sm:h-8 flex items-center justify-center cursor-pointer transition-all hover:bg-black/80 active:bg-black/90 touch-manipulation ${
              isMobile ? 'flex' : ''
            }`}
            onClick={(e) => {
              e.stopPropagation()
      ...
      <div className="p-2.5 sm:p-4 bg-[#17131D]">
        <div className="mb-2 sm:mb-3">
          <h3 className="text-sm sm:text-base font-medium text-text-primary mb-0.5 sm:mb-1 line-clamp-1">
            {item.title}
          </h3>
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 mb-2.5 sm:mb-4 text-xs">
          {item.is_available ? (
        ...
        <div className="flex gap-2 flex-wrap">
          {!item.is_available && (
            <a
              ...
              className="w-full text-center px-3 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-medium no-underline border border-[#f59e0b]/30 text-[#f59e0b] cursor-pointer inline-flex items-center justify-center gap-2 hover:bg-[#f59e0b]/10 hover:border-[#f59e0b]/50 active:bg-[#f59e0b]/15 transition-all touch-manipulation min-h-[40px] sm:min-h-[44px]"
            >
          ...
          {item.is_available && item.jellyfin_item_id && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-full text-center px-3 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-medium no-underline border border-[#8B6A9F]/50 text-[#B894D1] cursor-pointer inline-flex items-center justify-center gap-2 hover:bg-[#8B6A9F]/15 hover:border-[#B894D1]/70 active:bg-[#8B6A9F]/20 transition-all touch-manipulation min-h-[40px] sm:min-h-[44px]"
            >
